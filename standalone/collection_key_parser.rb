...
module CollectionKeyParser
  ...
  def self.get_requirements(key)
    ...
    # Consents (facets[6])
    consent_code = facets[6]
    if consent_code != 'N'
      requirements.consents_required.driver_license = true
      requirements.consents_required.drug_test = true
      requirements.consents_required.biometric = true
    end
